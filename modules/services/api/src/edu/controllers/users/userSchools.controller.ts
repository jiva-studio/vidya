...
export class UserSchoolsController {
   ...
   * the answer so a screen can say what it costs before it is done — the
   * places are already gone by the time the caller reads it, which is why the
   * confirmation belongs before the call and not after.
   */
