...
if [ "$VIDYA_GATEWAY_TLS_ENABLED" = "true" ]; then
  ...
  # A real hostname, so Caddy's automatic HTTPS treats this as a site to
  # request a certificate for (Let's Encrypt, falling back to ZeroSSL) rather
  # than a bare port. See README.md, "TLS certificates".
  ...
  # A bare port, not a domain: this is what keeps Caddy from ever treating
  # the local stand as a site to obtain a certificate for — no ACME attempt,
  # plain HTTP, same as `curl localhost:8080` today. See README.md,
  # "Local stand".
...
export VIDYA_GATEWAY_API_UPSTREAM VIDYA_GATEWAY_SITE_ADDRESS VIDYA_GATEWAY_HSTS_DIRECTIVE \
