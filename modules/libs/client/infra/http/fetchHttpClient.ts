...
/**
 * What the transport reports about a request that did not succeed.
 *
 * The library keeps no reporter of its own: an application that has one hands
 * it over, and one that has none loses nothing. The shape is the common
 * denominator of a breadcrumb, so a host can pass its reporter unwrapped.
 */
...
let trace: (event: HttpTrace) => void = () => undefined

/** Installed by the composition root; every client built afterwards reports. */
export const useHttpTrace = (reporter: (event: HttpTrace) => void): void => {
  trace = reporter
}
