...
describe('SchoolLogoField', () => {
  ...
  it('renders drop area when modelValue is empty', () => {
    const wrapper = mountWithApp(SchoolLogoField, {
      props: { modelValue: '' },
    })

    expect(wrapper.find('[data-test="logo-drop-area"]').exists()).toBe(true)
  })
  ...
  it('opens MediaPickerDialog when choose / picker trigger button is clicked', async () => {
    ...
    const trigger = wrapper.find(
      '[data-test="open-picker"], [data-test="logo-drop-area"], button[data-action="pick"]',
    )
