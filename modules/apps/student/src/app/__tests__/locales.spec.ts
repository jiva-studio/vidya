...
/**
 * Both languages, whole: a key in one bundle and not the other is a screen
 * that reads half in Russian, and a key used in a template and defined nowhere
 * is a screen that shows the key to the student.
 *
...
describe('the two bundles', () => {
  ...
  it('define every key the screens ask for', () => {
  ...
  it('leaves no Russian in a screen instead of a bundle', () => {
    const russian = all
      .filter((path) => /\.(ts|vue)$/.test(path) && !path.includes('__tests__'))
      // The start-up screen is mounted before the catalogue exists and carries
      // its own two sentences; there is nowhere else for them to live.
      .filter((path) => !path.endsWith('startupFailure.ts'))
      .filter((path) => /[Ѐ-ӿ]/.test(readFileSync(path, 'utf8')))
